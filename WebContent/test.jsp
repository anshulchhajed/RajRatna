...
if (rs1.next()) {
	...
	try {
		...
		PreparedStatement ps3 = Data.con
		.prepareStatement("insert into recentactivity (lastbillno) values(" + billno + ")");
		ps3.executeUpdate();
	} catch (Exception e) {
		...
		<div class="container-fluid text-center">
			<div class="row content">
				...
				<div class="col-sm-8 text-left">
					...
					<table width="100%">
						<tr>
							<td><input type="text" name="itemcodebarcode"
								id="itemcodebarcode" class="form-control" autofocus
								style="font-size: 20px; height: 40px; width: 100%;"></input></td>
							<td><input type="submit" class="btn btn-primary"
								name="search" id="search" value="Search"
								onclick="return check()"
								style="width: 100%; font-size: 20px; height: 40px;"></input></td>
						</tr>
					</table>
					...
					<table width="100%" id="custData">
						<tr>
							<td id="bill"><b>Bill No.<%=billno%></td>
							<td align="center"><b>Name: <%=name%></td>
							<td align="right"><b>Mobile No: <%=mob%></td>
						</tr>
					</table>
					<span id="cust"> </span>
					<div style="height: 50px">
						...
						if (billMsg != null) {
						%>
						<div id="dismissAlert3"
							class="alert alert-danger alert-dismissible">
							<button type="button" class="close" data-dismiss="alert">&times;</button>
							<center>
								<strong><%=billMsg%></strong>
							</center>
						</div>
						<%
						}
					...
					<div class="table-wrapper-scroll-y my-custom-scrollbar">
						<table id="dataTable"
							class="table table-bordered table-striped mb-0" width="100%">
							<thead>
								<tr>
							...
							<tbody align="center">
								...
								<tr align="center">
									<td><b><%=obj1.itemcode%></th>
											<td><%=obj1.itemtype%></td>
											<td><%=obj1.size%></td>
											<td><button type="submit" name="minus"
													value="<%=obj1.itemcode%>">-</button>&nbsp;<input
												type="text" size="1" style="width: 25px" id="qt<%=sno%>"
												readonly="readonly" name="quantity"
												value="<%=obj1.currQuantity%>" onblur="sendInfo()"></input>&nbsp;
												<button type="submit" name="add" value="<%=obj1.itemcode%>">+</button></td>
											<td><%=obj1.currPrice%></td>
											<td><button type="button" id="<%=obj1.itemcode%>">
													<img src="css/images/delete.png" height="20px" width="20px" />
												</button></td>
								</tr>
								<%
					...
					<table class="table table-bordered table-striped mb-0">
						...
						<tr align="center">
							...
							<td><input type="text" id="discount" name="discount"
								value=<%=discount%> style="width: 30px" onblur="updateInfo()" />
								%</td>
							<td id="disprice"><b>&#8377; <%=disprice%>/-
							</b></td>
							<td>&#8377;<input type="text" style="width: 50px;"
								id="adjust" name="adjustamount" value="<%=adjustamount%>"
								onblur="updateAmt()" /></td>
							<td id="amtpay"><b>&#8377; <%=amttopay%>/-</td>
						</tr>
						<tr>
							<td><input type="submit" class="btn btn-primary"
								onclick="disable()" id="new" name="submit" value="New"
								style="width: 100%; font-size: 15px; height: 30px;" /></td>
							<td><button type="button" class="btn btn-primary "
									data-toggle="modal" data-target="#myModal" id="open"
									value="Open"
									style="width: 100%; font-size: 15px; height: 30px;">Open</button></td>
							<td><input type="submit" class="btn btn-primary"
								onclick="disable()" id="delete" name="submit" value="Delete"
								style="width: 100%; font-size: 15px; height: 30px;" /></td>

							<td><input type="submit" class="btn btn-primary" id="save"
								name="submit" value="Save" onclick="validateField()"
								style="width: 100%; font-size: 15px; height: 30px;" /></td>
							<td><input type="submit" onclick="printDiv()"
								class="btn btn-primary" name="submit" id="print"
								value="Print Bill"
								style="width: 100%; font-size: 15px; height: 30px;" /></td>
							<td><input type="submit" class="btn btn-danger" id="clear"
								name="submit" value="Clear" onclick="clear()"
								style="width: 100%; font-size: 15px; height: 30px;" /></td>

						</tr>
					</table>
					<input type="hidden" name="billno" value="<%=billno%>"> <input
						type="hidden" name="itempurchased" value="<%=itemcount%>">
					...
					<script type="text/javascript">
						...
						setTimeout(function() {
							$('#dismissAlert,#dismissAlert1,#dismissAlert2')
									.fadeOut('fast');
						}, 1000);
					</script>
...
if (present && success == null) {%>
		...
		<Script type="text/javascript">
			...
			function updateInfo() {
				var str = (document.getElementById('totprice').innerHTML)
						.substring(5).replace("/-", "");
				var check = parseInt(document.getElementById('discount').value,
						10);
				...
				var val = (x * y) / 100;
				document.getElementById('disprice').innerHTML = "<b>&#8377; "
						+ val + "/-";
				document.getElementById('amtpay').innerHTML = "<b>&#8377; "
						+ (y - val) + "/-";
			}
			...
			function updateAmt() {
				...
				if (isNumeric) {
					...
					var y = parseInt((document.getElementById('totprice').innerHTML)
							.substring(5).replace("/-", ""));
					var z = parseInt((document.getElementById('disprice').innerHTML)
							.substring(5).replace("/-", ""));
					var val = (y - z) - x;
					...
					y = parseInt((document.getElementById('totprice').innerHTML)
							.substring(5).replace("/-", ""));
					z = parseInt((document.getElementById('disprice').innerHTML)
							.substring(5).replace("/-", ""));
		...
		<div class="modal" tabindex="-1" id="myModal"
			style="padding-top: 100px" role="dialog">
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					...
					<div class="modal-body">
						<input type="text" list="custdata" id="billInput" name="billInput"
							class="form-control" autocomplete="off"
							onkeypress="checkCustData()">
